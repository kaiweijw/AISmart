...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("AISmart") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "AISmart", DisplayName = "AISmart API", Resources = { "AISmart" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        {
            var swaggerRootUrl = configurationSection["AISmart_Swagger:RootUrl"]?.TrimEnd('/');
